...
describe('TeamCreateModal', () => {
  ...
  it('renders the workspace folder trigger and opens a directory dialog on click', async () => {
    mockShowOpen.mockResolvedValue(['/tmp/my-workspace']);
    render(<TeamCreateModal visible onClose={vi.fn()} onCreated={vi.fn()} />);

    const trigger = screen.getByTestId('team-create-workspace-trigger');
    expect(trigger).toBeInTheDocument();

    fireEvent.click(trigger);
    await vi.waitFor(() => expect(mockShowOpen).toHaveBeenCalledTimes(1));
  });
});
